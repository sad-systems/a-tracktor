...
export abstract class AbstractAnalyzer {
  ...
  protected setOptions(options?: IAbstractAnalyzerOptions) {
    ...
    this.connectDestination =
      options.connectDestination === undefined ? true : options.connectDestination;
  }
  ...
  protected initAudio() {
    ...
    if (this.mediaSource) {
      ...
      const audioSource = AbstractAnalyzer.createAudioSourceForElement(
        this.audioElement,
        this.connectDestination,
      );
  ...
  protected static findAudioSourceByElement(audioElement: HTMLMediaElement): IAudioSource | void {
    return AbstractAnalyzer.audioSources.find(
      (audioSource) => audioSource.audioElement === audioElement,
    );
  }
