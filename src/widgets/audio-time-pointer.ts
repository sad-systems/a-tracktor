 ...
 *
 * ![](media://images/audio-time-pointer.png)
 *
 * Widget to auto draw an audio time pointer control.
 *
 * The time pointer for `audioElement` will be auto drawn and appended inside the `viewElement` as child element.
 *
 * Example:
 * ```javascript
 *
 ...
 *
 * // Line time pointer:
 * const audioTimePointer1 = new AudioTimePointer(audioElement, viewElement);
 * // or
 * // Bar time pointer:
 * const audioTimePointer2 = new AudioTimePointer(audioElement, viewElement, { mode: 'bar', pointerStyle: { background: '#0f0a' });
 *
 * ```
 *
 * See more options at interface definition: {@link IAudioTimePointerOptions}
 *
 ...
 */
export enum EnumAudioTimePointerMode {
 ...
 */
export interface IAudioTimePointerOptions {
    /**
...
export class AudioTimePointer {
    ...
    protected setOptions(options?: IAudioTimePointerOptions) {
        ...
        this.pointerStyle = !this.pointerClass ? {...this.getPointerDefaultStyle(), ...options.pointerStyle} : undefined;
    ...
    protected movePointerToCurrentTime() {
        if (this.pointerElement) {
            let pos = this.audioElement.currentTime * this.viewElement.offsetWidth / this.audioElement.duration;
